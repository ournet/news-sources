...
export const SOURCES_SCHEMA_FILE_PATH = join(
  __dirname,
  "..",
  "data",
  "sources-schema.json"
...
export const SOURCES_FILENAMES = readdirSync(SOURCES_DIR_PATH).filter(
  (filename) => /^[a-z]{2}\.json$/.test(filename)
);
export const SOURCES_COUNTRIES = SOURCES_FILENAMES.map((filename) =>
  getCountryFromSourceFileName(filename)
);
